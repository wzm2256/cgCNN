# This file is modified from https://github.com/zilongzheng/STGConvNet
...
def loadVideoToFrames(data_path, syn_path, ffmpeg_loglevel = 'quiet'):
    ...
    for i in range(num_videos):
        ...
        if not os.path.exists(out_dir):
            os.makedirs(out_dir)
            # subprocess.call('ffmpeg -loglevel {} -i {} {}/%08d.png'.format(ffmpeg_loglevel,video_path, out_dir), shell=True)
            subprocess.call('ffmpeg -i {} {}/frame_%08d.png'.format(video_path, out_dir), shell=True)
    return num_videos
...
def saveSampleVideo(samples, out_dir, original=[], global_step=None, ffmpeg_loglevel='quiet', fps=10):
    ...
    for iv in range(num_video):
        ...
        for ifr in range(num_frames):
            ...
            plt.imsave("%s/frame_%08d.png" % (result_dir, ifr), saved_img)
        subprocess.call('ffmpeg -loglevel {} -r {} -i {}/frame_%08d.png -y {}/sample.gif'.format(
